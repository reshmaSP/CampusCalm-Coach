...
function AP() {
    ...
    const handlePsyDelete = (id) => {
        fetch(`${process.env.REACT_APP_BACKEND_URL}/deletePsy`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          ...
          .then((data) => {
            if (data.status === "success") {
              alert("Deleted Psychiatrist succesfully");
          ...
          <div className="psyinfowrapper">
            {displayedPsy.map((p) => (
              <div className="psyinfo" key={p[0]}>
                ...
                <div className="pdata" onClick={() => handlePsyDelete(p[0])}>
